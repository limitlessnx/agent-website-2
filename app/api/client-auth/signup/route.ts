...
export async function POST(request:NextRequest){
  try{
    ...
    const auth=await signUpClient(email,password,fullName,{ companyName:joiningOrganization?undefined:companyName, companySlug:joiningOrganization?undefined:companySlug, templateSlug:joiningOrganization?undefined:templateSlug, agentFamilyName:joiningOrganization?undefined:agentFamilyName, invitationToken:joiningOrganization?invitationToken:undefined });
    ...
    if(joiningOrganization){
      ...
      provisioned=await provisionClientOrganization({userId:auth.user.id,organizationName:companyName,organizationSlug:companySlug,templateSlug,agentFamilyName});
    ...
    const message=error instanceof Error?error.message:"Unable to create account."; const status=/already|duplicate|exists|registered|seat limit/i.test(message)?409:400; return NextResponse.json({error:message},{status});
  }
}
