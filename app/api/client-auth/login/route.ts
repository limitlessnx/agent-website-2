...
import { getMembershipForOrganization, getPrimaryMembership, setClientSession, setPendingClientSetupSession, signInClient } from "@/lib/client-auth";
...
export async function POST(request:NextRequest){
  try{
    ...
    const metadata=auth.user.user_metadata||{}, fullName=String(metadata.full_name||"").trim(), companyName=String(metadata.company_name||"").trim(), invitationToken=String(metadata.invitation_token||"").trim();
