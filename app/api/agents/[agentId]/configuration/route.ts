...
const clientChannels = new Set(["whatsapp", "email", "web", "telegram", "voice"]);

function normalizeChannels(value: unknown) {
  if (!Array.isArray(value)) return [];
  return Array.from(new Set(value.map((channel) => String(channel).trim().toLowerCase()).filter((channel) => clientChannels.has(channel))));
}
...
export async function GET(_: NextRequest, context: { params: Promise<{ agentId: string }> }) {
  try {
    ...
    const { data, error } = await supabase.from("agents")
      ...
      .eq("id", agentId).eq("organization_id", session.organizationId).single();
    if (error) throw error;
    return NextResponse.json({ agent: { ...data, communication_channels: normalizeChannels(data.communication_channels) } });
  } catch (error) {
...
export async function PUT(request: NextRequest, context: { params: Promise<{ agentId: string }> }) {
  try {
    ...
    const supabase = await createClient();
    const { data: current, error: readError } = await supabase.from("agents").select("status,configuration").eq("id", agentId).eq("organization_id", session.organizationId).single();
    ...
    const configuration = { ...(current.configuration || {}), qualification_questions: qualificationQuestions, working_hours: workingHours, channel_instructions: channelInstructions, configured_by: session.userId, configured_at: new Date().toISOString() };
    const { data, error } = await supabase.from("agents").update({
      name, description: String(body.description || "").trim() || null, system_prompt: systemPrompt, language, temperature,
      communication_channels: channels, escalation_rules: escalationRules, human_handoff_destination: handoff,
      configuration, status: "testing", updated_at: new Date().toISOString(),
    }).eq("id", agentId).eq("organization_id", session.organizationId).select("id,status,updated_at").single();
    if (error) throw error;
