...
export async function POST(request: Request) {
  try {
    ...
    const quoteItems = selections.map((item) => ({
      organization_id: session.organizationId,
      quote_id: quote.id,
      agent_selection_id: item.id,
      item_type: "agent",
      item_key: item.agent_key,
    ...
    if (!secret) return NextResponse.json({ error: "Payment provider is not configured.", quote_id: quote.id }, { status: 503 });
    ...
    const paystackResponse = await fetch("https://api.paystack.co/transaction/initialize", {
      ...
      body: JSON.stringify({
        ...
        currency,
        reference,
        callback_url: `${appUrl}/portal/payment/success?reference=${reference}`,
        metadata: { organization_id: session.organizationId, quote_id: quote.id, user_id: session.userId },
      }),
    });
    ...
    const { error: paymentError } = await admin.from("payment_attempts").insert({
      ...
      provider_payload: initialized.data,
