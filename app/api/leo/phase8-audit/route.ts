...
import { auditLeoPhase8 } from "@/lib/leo-phase8-audit";
...
export async function GET() {
  ...
  try {
    const audit = await auditLeoPhase8({ identity });
    return NextResponse.json({ ok: audit.status !== "fail", audit }, { headers: { "cache-control": "no-store" }, status: audit.status === "fail" ? 409 : 200 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Phase 8 audit failed." }, { status: 500 });
  }
}
