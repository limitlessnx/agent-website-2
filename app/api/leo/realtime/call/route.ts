...
function upstreamErrorBody(value: string) {
  try {
    ...
    if (parsed?.error) return { message: parsed.error.message || "OpenAI rejected the realtime session.", type: parsed.error.type || null, code: parsed.error.code || null };
  } catch {}
...
export async function POST(request: NextRequest) {
  ...
  const session = {
    type: "realtime", model, instructions: voiceInstructions(identity), output_modalities: ["audio"], audio: { output: { voice } },
    tools: [
  ...
  const headers = new Headers({ "content-type": "application/sdp", "cache-control": "no-store", "x-leo-realtime-model": model, "x-leo-realtime-voice": voice });
