...
export async function POST(request: Request) {
  ...
  if (!isValidFlutterwaveWebhook(rawBody, request.headers)) return NextResponse.json({ error: "Invalid webhook signature." }, { status: 401 });
  ...
  try {
    const existing = await supabaseRest<any[]>(`payment_webhook_events?provider=eq.flutterwave&external_event_id=eq.${encodeURIComponent(externalEventId)}&limit=1`);
    if (existing.length) return NextResponse.json({ received: true, duplicate: true });

    await supabaseRest("payment_webhook_events", { method: "POST", body: JSON.stringify({ provider: "flutterwave", external_event_id: externalEventId, event_type: eventType, signature_valid: true, payload }) });

    if (eventType !== "charge.completed" || !data?.tx_ref || !data?.id) return NextResponse.json({ received: true });
