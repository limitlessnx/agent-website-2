...
export default function PricingClient() {
  ...
  const checkoutSlug = active.key === "business-plus" ? null : activePrice?.slug ?? checkoutSlugByPlan[active.key];
  const checkoutHref = active.key === "business-plus"
    ? evaluationHref
    : `/checkout?plan=${encodeURIComponent(checkoutSlug || checkoutSlugByPlan[active.key])}${billingTerm === "monthly" ? "" : `&term=${encodeURIComponent(billingTerm)}`}`;
  const planCtaLabel = active.key === "business-plus"
    ? "Configure Business+"
    ...
    <main className="quantix-home pricing-page-shell">
      <section className="brand-section pricing-page-hero">
        <div className="brand-shell">
          <div className="brand-heading pricing-page-heading">
            ...
            <p>Basic, Plus, Business and Business+ stay consistent across industries. Choose monthly billing or prepay a longer term to reduce the total cost.</p>
            {canViewInternational ? (
      ...
      <section className="brand-section pricing-page-content">
        <div className="brand-shell">
          ...
          <p className="pricing-footnote">Prepaid discounts apply to the implementation plus the selected prepaid renewal period. Third-party messaging, email and provider usage may still be subject to fair-use limits or additional usage charges depending on volume and provider costs.</p>
        </div>
      </section>
