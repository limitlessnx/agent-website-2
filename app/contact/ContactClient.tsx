        ...
        <div className="contact-shell contact-grid">
          ...
          <motion.section className="contact-form-card" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .55, delay: .12 }} aria-labelledby="contact-form-title">
            <div className="contact-form-heading">
              ...
              <p>Give us enough context to understand the operation. Submissions enter the same controlled lead-intake backbone used for Business AI Evaluations.</p>
            </div>

            <div style={fieldWrap}>
              <label style={labelStyle} htmlFor="name">Full name *</label>
              <input id="name" autoComplete="name" required value={form.name} onChange={e => setForm(current => ({ ...current, name: e.target.value }))} style={inputStyle("name")} placeholder="Your name" />
              {errMsg("name")}
            </div>
            ...
            <div className="contact-two-col" style={fieldWrap}>
              <div>
                ...
                <input id="email" type="email" autoComplete="email" required value={form.email} onChange={e => setForm(current => ({ ...current, email: e.target.value }))} style={inputStyle("email")} placeholder="you@company.com" />
                {errMsg("email")}
              </div>
              <div>
                <label style={labelStyle} htmlFor="phone">Phone / WhatsApp</label>
                <input id="phone" type="tel" autoComplete="tel" value={form.phone} onChange={e => setForm(current => ({ ...current, phone: e.target.value }))} style={inputStyle("phone")} placeholder="+234 ..." />
              </div>
            </div>

            <div style={fieldWrap}>
              <label style={labelStyle} htmlFor="businessName">Business name *</label>
              <input id="businessName" autoComplete="organization" required value={form.businessName} onChange={e => setForm(current => ({ ...current, businessName: e.target.value }))} style={inputStyle("businessName")} placeholder="Your company or organization" />
              {errMsg("businessName")}
            </div>

            <div style={fieldWrap}>
              <label style={labelStyle} htmlFor="businessType">Business type *</label>
              <select id="businessType" required value={form.businessType} onChange={e => setForm(current => ({ ...current, businessType: e.target.value }))} style={{ ...inputStyle("businessType"), cursor: "pointer" }}>
            ...
            <div className={`contact-consent ${errors.consent ? "has-error" : ""}`}>
              <label htmlFor="consent">
                ...
                />
                <span>I agree that Fluxknight may contact me about this business request. This does not subscribe me to marketing messages.</span>
              </label>
              {errMsg("consent")}
            </div>

            {status === "error" && (
              <div className="contact-error"><AlertCircle size={17} /><span>We could not safely record the requirement. Please try again or use the direct email address.</span></div>
            )}
