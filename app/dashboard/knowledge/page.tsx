    ...
    <main className="admin-page knowledge-page">
      <header className="admin-page-header">
        <div>
          ...
          <p>Organize verified business information before agents are allowed to answer from it.</p>
        </div>
      </header>

      <section className="admin-metric-grid knowledge-metrics">
        ...
        <article className="admin-metric-card"><p><FileText size={15} /> Sources</p><strong>{sourceCount}</strong><span>Registered source records</span></article>
      </section>

      <section className="admin-panel knowledge-collections">
        <div className="admin-panel-header">
          <div><h2>Knowledge collections</h2><p>Properties, FAQs, pricing, policies and operational guides live here.</p></div>
          <Database size={18} />
        </div>
        <div className="admin-list knowledge-list">
          {knowledge.map((item) => (
            <div className="admin-list-row knowledge-row" key={item.id}>
              <div className="knowledge-row-copy">
                <strong>{item.name}</strong>
                <span>{item.description || "No description"}</span>
                <small>{item.source_count} {Number(item.source_count) === 1 ? "source" : "sources"}</small>
              </div>
              <em className="knowledge-status">{humanize(item.status)}</em>
            </div>
          ))}
