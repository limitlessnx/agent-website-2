...
export default async function RetentionPage() {
  ...
  return (
    <main className="admin-page retention-page">
      <header className="admin-page-header">
        <div>
          <p className="admin-kicker">Retention intelligence</p>
          <h1>Churn Prevention & Win-back</h1>
          <p>Account intent, declining usage, integration reliability and support pressure in one retention view.</p>
        </div>
      ...
      <div className="admin-metric-grid retention-metrics">
      ...
      <section className="admin-panel retention-queue">
        <div className="admin-panel-header">
          <div><h2>Retention queue</h2><p>Signals are advisory. No discount, account change or outreach is sent automatically.</p></div>
          <AlertTriangle size={18} />
        </div>
        <div className="admin-list">
          {snapshots.map((snapshot) => (
            <article className="admin-list-row retention-account" key={snapshot.organizationId}>
              <div className="retention-account-main">
                <div className="retention-account-heading">
                ...
                {snapshot.signals.length ? (
                  <div className="admin-list retention-signals">
                ...
                <span className="admin-muted retention-recommendation">Recommended: {snapshot.recommendedAction}</span>
              </div>
      ...
      <section className="admin-panel retention-guardrails">
        <div className="admin-panel-header"><div><h2>Retention guardrails</h2><p>Retention interventions need context, not panic automation.</p></div><Target size={18} /></div>
        <div className="admin-list">
