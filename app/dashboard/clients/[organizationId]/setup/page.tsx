...
function stageState(complete: boolean) { return complete ? "admin-status live" : "admin-status warning"; }
    ...
    <main className="admin-page">
      ...
      <section className="admin-panel">
        <div className="admin-list" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", display: "grid" }}>
          {steps.map((step, index) => <div className="admin-list-row compact" key={step.label}><span>{step.complete ? <CheckCircle2 size={16} /> : <CircleDashed size={16} />}</span><div><strong>{index + 1}. {step.label}</strong><span>{step.complete ? "Complete" : "Needs attention"}</span></div></div>)}
        </div>
      </section>

      <section className="admin-panel" id="business">
        <div className="admin-panel-header"><div><h2>1. Business details</h2><p>The tenant identity and operating context used across every allocated agent.</p></div><span className={stageState(businessComplete)}>{businessComplete ? "Ready" : "Needs details"}</span></div>
      ...
      <section className="admin-panel" id="agents">
        <div className="admin-panel-header"><div><h2>2. Agent allocation and workspace build</h2><p>Select any reusable marketplace agents this organization needs. Super Admin allocation is not restricted by the client's plan.</p></div><Bot size={18} /></div>
        <AgentAllocationControl organizationId={organizationId} embedded />
        {agents.length ? <div className="admin-list" style={{ marginTop: 16 }}>{agents.map((agent) => <div className="admin-list-row compact" key={agent.id}><Bot size={15} /><div><strong>{agent.name}</strong><span>{humanize(agent.agent_type || "general agent")}</span></div><em className={agent.status === "active" ? "good" : "muted"}>{humanize(agent.status)}</em></div>)}</div> : <p className="admin-empty">No tenant agents are provisioned yet.</p>}
      </section>
