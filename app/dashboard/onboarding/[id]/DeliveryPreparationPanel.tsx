...
export default function DeliveryPreparationPanel({
  ...
  async function prepareDelivery() {
    ...
    try {
      ...
      setMessage(
        `Delivery prepared: ${summary?.agents_updated ?? 0} agents updated, ${summary?.workflows_updated ?? 0} workflows mapped, and the client notification drafted.`,
      );
      router.refresh();
    } catch (error) {
  ...
  return (
    <section className="admin-panel">
      <div className="admin-panel-header">
        <div>
          <h2>Prepare managed delivery</h2>
          <p>Turn the approved onboarding answers into agent configuration, workflow metadata, organization knowledge, and a workspace-ready notification draft.</p>
        </div>
        <button className="admin-button" type="button" disabled={busy || !organizationId} onClick={prepareDelivery}>
          {busy ? "Preparing..." : "Prepare delivery"}
        </button>
      </div>
