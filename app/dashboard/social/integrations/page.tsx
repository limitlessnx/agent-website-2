...
export default async function SocialIntegrationsPage({
  ...
  const loginConfigurationId = String(
    credentials?.login_configuration_id ||
      config.login_configuration_id ||
      "",
  );
  const preferredPageId = String(
    credentials?.preferred_page_id || config.preferred_page_id || "",
  );
  const preferredInstagramAccount = String(
    credentials?.preferred_instagram_account ||
      config.preferred_instagram_account ||
      "",
  );
  ...
  const success =
    ...
    params.meta === "sync-complete";
  const error = typeof params.error === "string" ? params.error : null;
  const syncStatus =
    typeof params.sync_status === "string" ? params.sync_status : null;
  const accountSamples =
    typeof params.account_samples === "string" ? params.account_samples : null;
  const postSamples =
    typeof params.post_samples === "string" ? params.post_samples : null;
    ...
    <main className="admin-page">
      <header className="admin-page-header">
        <div>
          ...
          <p>
            Configure and connect the platforms Flux Social uses for publishing,
            analytics and learning.
          </p>
        </div>
        <Link href="/dashboard/social" className="admin-btn secondary">
          Back to Social
        </Link>
      </header>
      ...
      {success ? (
        <section
          ...
          style={{ borderColor: "rgba(34,197,94,.45)" }}
        >
          <strong>
            {params.meta === "connected"
              ? "Meta connected successfully."
              : params.meta === "sync-complete"
                ? "Meta analytics sync completed."
                : "Meta app credentials saved securely."}
          </strong>
          <p style={{ marginTop: 8 }}>
            {params.meta === "connected"
              ? "Facebook and Instagram analytics can now sync into Flux Social."
              : params.meta === "sync-complete"
                ? `Ingestion status: ${syncStatus || "unknown"}. Account samples: ${accountSamples || "0"}. Post samples: ${postSamples || "0"}.`
        ...
        <section className="admin-panel">
          <div className="admin-panel-header">
            <div>
              ...
              <p>
                Enter the Meta Developer App credentials here. The App Secret is
                stored in Supabase Vault and is never shown again.
              </p>
            </div>
            <span
              className={
                configured ? "admin-status live" : "admin-status warning"
              }
            >
        ...
        <section className="admin-panel">
          <div className="admin-panel-header">
            <div>
              ...
              <p>
                Authorize the Facebook Page and linked Instagram Business
                account Flux Social should use.
              </p>
            </div>
            <span
              className={
          ...
          <div className="admin-list" style={{ marginBottom: 20 }}>
            <div className="admin-list-row">
              <div>
                ...
                <span>{String(config.page_name || "Not connected")}</span>
              </div>
            ...
            <div className="admin-list-row">
              ...
              <em>
                {config.instagram_business_account_id ? "linked" : "pending"}
              </em>
            </div>
            <div className="admin-list-row">
              <div>
                <strong>Preferred Page</strong>
                <span>
                  {preferredPageId || preferredInstagramAccount || "Not set"}
                </span>
              </div>
              <em>
                {preferredPageId || preferredInstagramAccount
                  ? "targeted"
                  : "auto"}
              </em>
            </div>
            <div className="admin-list-row">
              <div>
                ...
                <span>
                  {loginConfigurationId ? "Configured" : "Not configured"}
                </span>
              </div>
            ...
            <div className="admin-list-row">
              ...
              <em>{String(health.state || health.status || "pending")}</em>
            </div>
            <div className="admin-list-row">
              <div>
                ...
                <span>
                  {integration?.last_connected_at
                    ? new Date(integration.last_connected_at).toLocaleString()
                    : "Never"}
                </span>
              </div>
      ...
      <section className="admin-panel">
        <div className="admin-panel-header">
          <div>
            ...
            <p>
              Fluxknight keeps provider setup inside the product while sensitive
              credentials stay protected behind the server.
            </p>
          </div>
        </div>
        <div className="admin-list">
          <div className="admin-list-row">
            <div>
              ...
              <span>Visible configuration used to start Meta OAuth.</span>
            </div>
            <em>dashboard</em>
          </div>
          <div className="admin-list-row">
            <div>
              <strong>Login Configuration ID</strong>
              <span>Stored for Meta Business Login when that flow is enabled.</span>
            </div>
            <em>dashboard</em>
          </div>
          <div className="admin-list-row">
            <div>
              <strong>Preferred Page</strong>
              <span>
                Page or Instagram target used when Meta returns multiple assets.
              </span>
            </div>
            <em>dashboard</em>
          </div>
          <div className="admin-list-row">
            <div>
              ...
              <span>
                Encrypted in Supabase Vault and never rendered back to the browser.
              </span>
            </div>
            <em>vault</em>
          </div>
          <div className="admin-list-row">
            <div>
              ...
              <span>
                Created after OAuth and stored in the same protected credential record.
              </span>
            </div>
            <em>vault</em>
          </div>
          <div className="admin-list-row">
            <div>
              ...
              <span>
                Normalized into Flux Social metrics and learning snapshots.
              </span>
            </div>
