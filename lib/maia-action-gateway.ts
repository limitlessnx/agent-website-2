...
async function resolveActionWorkflow() {
  try {
    const exact = await getN8nWorkflow(ACTION_WORKFLOW_ID);
    if (exact?.id) return exact;
  } catch {
    // Fall back to name lookup if the workflow was recreated with another ID.
  }
...
function targets(connection?: WorkflowConnection) {
...
export async function ensureMaiaActionWebhook() {
  ...
  const cleanedNodes = nodes.filter(
    (node) => node.name !== ACTION_WEBHOOK_NODE && node.name !== ACTION_NORMALIZE_NODE,
  );

  const baseX = Number(entryNode.position?.[0] || 0);
  const baseY = Number(entryNode.position?.[1] || 0);
  const webhook: WorkflowNode = {
    id: webhookNode?.id || crypto.randomUUID(),
    name: ACTION_WEBHOOK_NODE,
    type: "n8n-nodes-base.webhook",
    typeVersion: 2,
    position: [baseX - 440, baseY + 180],
    webhookId: String(webhookNode?.webhookId || crypto.randomUUID()),
    parameters: {
      httpMethod: "POST",
      path: ACTION_WEBHOOK_PATH,
      responseMode: "onReceived",
      options: {},
    },
  };

  const normalize: WorkflowNode = {
    id: normalizeNode?.id || crypto.randomUUID(),
    name: ACTION_NORMALIZE_NODE,
  ...
  const webhookConnection: WorkflowConnection = {
    main: [[{ node: ACTION_NORMALIZE_NODE, type: "main", index: 0 }]],
  };
  const normalizeConnection: WorkflowConnection = {
    main: [[{ node: ACTION_ENTRY_NODE, type: "main", index: 0 }]],
  };
  ...
  const needsUpdate =
    ...
    JSON.stringify(connections[ACTION_WEBHOOK_NODE] || {}) !== JSON.stringify(webhookConnection) ||
    JSON.stringify(connections[ACTION_NORMALIZE_NODE] || {}) !== JSON.stringify(normalizeConnection);

  if (needsUpdate) {
    const nextConnections = { ...connections };
    delete nextConnections[ACTION_WEBHOOK_NODE];
    delete nextConnections[ACTION_NORMALIZE_NODE];
    const updated = await updateN8nWorkflow(workflow.id, {
      name: workflow.name,
      nodes: [...cleanedNodes, webhook, normalize],
      connections: {
        ...nextConnections,
        [ACTION_WEBHOOK_NODE]: webhookConnection,
        [ACTION_NORMALIZE_NODE]: normalizeConnection,
      },
      settings: {
        ...(workflow.settings || {}),
  ...
  return {
    ...
    webhookPath: ACTION_WEBHOOK_PATH,
    entryNode: ACTION_ENTRY_NODE,
...
function buildCampaignInput(command: MaiaCampaignAction) {
  ...
  return {
    ...
    chat_id: command.createdBy || "fluxknight_dashboard",
    user_id: command.createdBy || "fluxknight_dashboard",
...
function extractNodeJson(execution: Record<string, any>, nodeName: string) {
...
async function waitForCampaignExecution(workflowId: string, commandId: string, startedAfter: number) {
  ...
  while (Date.now() < deadline) {
    ...
    if (error || ["error", "crashed", "canceled"].includes(status)) {
      ...
      throw new Error(`Maia action execution ${execution.id} failed at ${node}: ${detail}`);
    }

    if (execution.finished || status === "success") {
      const path = Object.keys(resultData.runData || {});
      const summary = extractNodeJson(execution, CAMPAIGN_SUMMARY_NODE);
      if (!summary) {
        throw new Error(
          `Maia action execution ${execution.id} finished without ${CAMPAIGN_SUMMARY_NODE}. Executed: ${path.join(" -> ") || "none"}.`,
        );
      }
      return {
  ...
  throw new Error(
    seenId
      ? `Maia action execution ${seenId} did not finish within ${EXECUTION_TIMEOUT_MS / 1000} seconds.`
      : `No Maia action execution was found for command ${commandId}.`,
  );
}
...
export async function dispatchMaiaCampaignAction(command: MaiaCampaignAction) {
  ...
  if (!response.ok) {
    throw new Error(
      `Maia action webhook rejected the campaign: ${response.status}${responseText ? ` ${responseText}` : ""}`,
    );
  }
  ...
  return {
    ...
    path: execution.path,
    summary,
    attempted: Number(summary.attempted || command.recipients.length),
    accepted: Number(summary.accepted_by_whatsapp_api || summary.submitted || 0),
    failed: Number(summary.immediate_failed || 0),
    ...
    acceptedRecipients: summary.accepted_recipients || [],
    failedRecipients: summary.failed_recipients || [],
    status: String(summary.status || "submitted"),
    message: String(summary.message || "Campaign submitted to WhatsApp."),
  };
}
