...
async function loadModels(organizationId: string) {
  ...
  const { data: assignments } = await admin
    .from("organization_ai_model_assignments")
  ...
  const { data } = await admin
    ...
    .select("id,provider,model_key,display_name,capabilities")
    .in("id", ids)
    .eq("status", "active");
...
async function chooseModel(organizationId: string, profile: RuntimeProfile, message: string) {
  ...
  if (process.env.OPENAI_API_KEY?.trim() && fallbackKey) {
    return { id: "fallback", provider: "openai", model_key: fallbackKey, display_name: fallbackKey, capabilities: { text: true, tools: true } } as Model;
  }
  return null;
}
...
async function loadAgent(organizationId: string, agentId: string) {
  ...
  const { data, error } = await admin
    .from("agents")
    .select("id,organization_id,name,slug,description,system_prompt,agent_type,configuration,communication_channels,status")
    .eq("id", agentId)
    .eq("organization_id", organizationId)
    .maybeSingle();
  ...
  return [
    {
      ...
      description: "Read the current tenant's approved business profile, onboarding submission and approved knowledge sources.",
      parameters: { type: "object", additionalProperties: false, properties: {} },
      execute: async (_input, ctx) => loadBusinessContext(ctx.organizationId),
    },
    {
...
function modelMessages(systemPrompt: string, history: Array<{ role: string; content: string }>, message: string) {
  return [{ role: "system", content: systemPrompt }, ...history, { role: "user", content: message }];
}
...
async function callOpenAI(model: Model, messages: any[], tools: ToolDefinition[]) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: model.model_key, temperature: 0.2, messages, tools: toolSchemas(tools), tool_choice: "auto" }),
...
export async function runMaia(input: MaiaRuntimeInput) {
  ...
  while (steps < Math.max(1, Math.min(profile.max_steps, 20))) {
    ...
    if (!calls.length) {
      finalText = text(assistant.content) || "I could not produce a useful response from the available tenant context.";
      break;
    }
    for (const call of calls) {
      ...
      if (!tool) {
        const output = { error: "Tool is not available to this agent." };
