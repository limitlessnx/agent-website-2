...
export const PUBLIC_LEO_POLICY = [
  ...
  "VOICE EMAIL SAFETY: when an email is spoken, never treat the first transcription as confirmed. Stage the normalized candidate, read that exact email back clearly, ask whether it is correct, and wait for the visitor's next spoken turn.",
  "Only after the visitor explicitly confirms the exact read-back email may voice lead capture use email_confirmed=true. If the visitor corrects any character or word, replace the candidate and repeat confirmation.",
  "LEAD CAPTURE: once name and a confirmed email are available, use leo.public.lead.capture exactly once. Do not repeatedly ask for captured details.",
