...
export type LeoSessionState = { id: string; persisted: boolean; visibility: LeoConversationVisibility; leadProfile?: PublicLeoLeadProfile; leadCaptured: boolean; leadId?: string | null };
...
export async function getOrCreateLeoSession(input: { identity: LeoIdentity; sessionId?: string; pageContext?: unknown; visibility?: unknown }): Promise<LeoSessionState> {
  ...
  if (requestedId) {
    ...
    if (existing[0]) {
      ...
      return { id: existing[0].id, persisted: true, visibility: existing[0].visibility as LeoConversationVisibility, ...leadState(existing[0]) };
    }
  }
  const id = randomUUID();
  ...
  return { id: created[0]?.id || id, persisted: Boolean(created[0]?.id), visibility, leadCaptured: false, leadProfile: undefined, leadId: null };
}
