...
export function registerProductionRuntimeExecutors(registry: RuntimeToolRegistry) {
  ...
  register(registry, "leo.workflow.inspect", async (args, context) => {
    ...
    const rows = await supabaseServerRequest<Record<string, unknown>[]>(`workflow_registry?select=id,workflow_key,name,description,provider,external_workflow_id,status,current_version,timeout_seconds,max_retries,last_run_at,last_success_at,last_error_at,environment,agent_id,updated_at&organization_uuid=eq.${encodeURIComponent(organizationId)}${filter ? `&${filter}` : ""}&order=updated_at.desc&limit=50`);
    return rows;
  });
  ...
  register(registry, "leo.crm.leads.update", async (args, context) => {
    ...
    const rows = await supabaseServerRequest<Record<string, unknown>[]>(`crm_leads?id=eq.${encodeURIComponent(leadId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify(body), headers: { Prefer: "return=representation" } });
  ...
  register(registry, "leo.agent.pause", async (args, context) => {
    ...
    const agentId = text(args.agent_id || args.agentId || context.agentId);
    if (!agentId) throw new Error("agent_id is required.");
    const rows = await supabaseServerRequest<Record<string, unknown>[]>(`agents?id=eq.${encodeURIComponent(agentId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "paused", updated_at: new Date().toISOString() }), headers: { Prefer: "return=representation" } });
    if (!rows[0]) throw new Error("Agent was not found in this organization.");
  ...
  register(registry, "leo.agent.resume", async (args, context) => {
    ...
    const agentId = text(args.agent_id || args.agentId || context.agentId);
    if (!agentId) throw new Error("agent_id is required.");
    const rows = await supabaseServerRequest<Record<string, unknown>[]>(`agents?id=eq.${encodeURIComponent(agentId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "active", updated_at: new Date().toISOString() }), headers: { Prefer: "return=representation" } });
    if (!rows[0]) throw new Error("Agent was not found in this organization.");
    return { id: rows[0].id, status: rows[0].status };
  });
  ...
  register(registry, "leo.platform.tenant.pause", async (args, context) => {
    ...
    const rows = await supabaseServerRequest<Record<string, unknown>[]>(`organizations?id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "paused", updated_at: new Date().toISOString() }), headers: { Prefer: "return=representation" } });
  ...
  register(registry, "leo.platform.tenant.resume", async (args, context) => {
    if (context.identity.scope !== "super_admin") throw new Error("Platform tenant control is restricted to Super Admin.");
    const organizationId = requestedOrganization(args, context);
    const rows = await supabaseServerRequest<Record<string, unknown>[]>(`organizations?id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "active", updated_at: new Date().toISOString() }), headers: { Prefer: "return=representation" } });
    if (!rows[0]) throw new Error("Organization was not found.");
    return { id: rows[0].id, status: rows[0].status };
  });

  const workflowStatusExecutor = (status: "active" | "inactive"): RuntimeToolExecutor => async (args, context) => {
    ...
    const rows = await supabaseServerRequest<Record<string, unknown>[]>(`workflow_registry?${filter}`, { method: "PATCH", body: JSON.stringify({ status, updated_at: new Date().toISOString() }), headers: { Prefer: "return=representation" } });
  ...
  register(registry, "leo.platform.workflow.deactivate", workflowStatusExecutor("inactive"));
