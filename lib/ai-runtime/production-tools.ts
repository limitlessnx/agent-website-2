...
function requireOrganization(context: RuntimeToolExecutorContext): string {
  ...
  if ((context.identity.scope === "tenant" || context.identity.scope === "internal_service") && context.identity.organizationId !== organizationId) {
    throw new Error("Cross-organization production execution is forbidden.");
  }
  return organizationId;
}

function requireId(input: Record<string, unknown>, ...keys: string[]): string {
...
function safeLeadPatch(input: Record<string, unknown>) {
  ...
  if (!Object.keys(patch).length) throw new Error("No permitted lead fields were supplied.");
...
async function mappedWorkflow(input: Record<string, unknown>, context: RuntimeToolExecutorContext, workflowKey: string, consequential: boolean) {
  ...
  const executor = new LeoN8nExecutor({
    ...base,
    n8n: {
      ...base.n8n,
      enabled: true,
    ...
    record: async (result) => {
      await supabaseServerRequest("runtime_progress_events", {
        method: "POST",
        body: JSON.stringify({ organization_id: organizationId, execution_id: context.executionId, event_type: `tool.workflow.${result.status}`, message: `Production workflow ${workflowKey} finished with ${result.status}.`, payload: { workflow_key: workflowKey, attempts: result.attempts.length, tool_key: context.toolKey } }),
      }).catch(() => null);
    },
  });
  const result = await executor.execute(workflowKey, {
    ...
    input,
    approval: consequential ? { approved: true, approvedBy: context.authorization.approvedBy || "runtime-approval-ledger", approvedAt: context.authorization.approvedAt || new Date().toISOString() } : undefined,
...
export function registerProductionRuntimeTools(registry: RuntimeToolRegistry) {
  const register = (key: string, executor: (input: Record<string, unknown>, context: RuntimeToolExecutorContext) => Promise<unknown>) => registry.registerExecutor(key, executor);
  ...
  register("leo.agent.inspect", async (input, context) => {
    const organizationId = requireOrganization(context);
    const agentId: string = requireId(input, "agent_id", "id");
    const rows = await supabaseServerRequest<unknown[]>(`agents?select=id,name,slug,description,status,current_version,agent_type,ai_model,language,communication_channels,escalation_rules,updated_at&id=eq.${encodeURIComponent(agentId)}&organization_id=eq.${encodeURIComponent(organizationId)}&limit=1`);
  ...
  register("leo.workflow.inspect", async (input, context) => {
    ...
    const rows = await supabaseServerRequest<unknown[]>(`workflow_registry?select=id,workflow_key,name,description,provider,status,current_version,timeout_seconds,max_retries,last_run_at,last_success_at,last_error_at,updated_at&organization_uuid=eq.${encodeURIComponent(organizationId)}&${filter}&limit=1`);
    if (!rows[0]) throw new Error("Workflow was not found in this organization.");
    return rows[0];
  });
  ...
  register("leo.crm.leads.read", async (input, context) => {
    const organizationId = requireOrganization(context);
    const leadId = text(input.lead_id) || text(input.id);
    const stage = text(input.stage);
    const limit = Math.max(1, Math.min(100, numeric(input.limit, 25)));
    const extra = leadId ? `&id=eq.${encodeURIComponent(leadId)}` : stage ? `&stage=eq.${encodeURIComponent(stage)}` : "";
  ...
  register("leo.crm.leads.update", async (input, context) => {
    ...
    const rows = await supabaseServerRequest<unknown[]>(`crm_leads?id=eq.${encodeURIComponent(leadId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify(safeLeadPatch(input)) });
    if (!rows[0]) throw new Error("Lead update did not affect a tenant-scoped record.");
    return rows[0];
  });

  register("leo.crm.followup.prepare", async (input, context) => {
    const organizationId = requireOrganization(context);
    const rows = await supabaseServerRequest<unknown[]>("crm_tasks", { method: "POST", body: JSON.stringify({ organization_id: organizationId, lead_id: requireId(input, "lead_id"), assigned_agent_id: context.agentId || null, task_type: "runtime_follow_up", title: text(input.title) || "AI prepared follow-up", description: text(input.message) || text(input.content) || null, status: "prepared", due_at: text(input.due_at) || null, metadata: { execution_id: context.executionId, source: "phase13_runtime" } }) });
    return rows[0] || null;
  });
  ...
  register("leo.appointment.manage", async (input, context) => {
    ...
    const rows = await supabaseServerRequest<unknown[]>(`crm_tasks?id=eq.${encodeURIComponent(taskId)}&organization_id=eq.${encodeURIComponent(organizationId)}&task_type=eq.appointment_booking`, { method: "PATCH", body: JSON.stringify(patch) });
    if (!rows[0]) throw new Error("Appointment was not found in this organization.");
    return rows[0];
  });
  ...
  register("leo.agent.pause", async (input, context) => {
    ...
    const rows = await supabaseServerRequest<unknown[]>(`agents?id=eq.${encodeURIComponent(agentId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "paused", updated_at: new Date().toISOString() }) });
    if (!rows[0]) throw new Error("Agent was not found in this organization.");
    return rows[0];
  });
  ...
  register("leo.agent.resume", async (input, context) => {
    const organizationId = requireOrganization(context);
    const agentId = requireId(input, "agent_id", "id");
    const rows = await supabaseServerRequest<unknown[]>(`agents?id=eq.${encodeURIComponent(agentId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "active", updated_at: new Date().toISOString() }) });
    if (!rows[0]) throw new Error("Agent was not found in this organization.");
    return rows[0];
  });
  ...
  register("leo.support.request_admin_repair", async (input, context) => {
    const organizationId = requireOrganization(context);
    const rows = await supabaseServerRequest<unknown[]>("handoff_requests", { method: "POST", body: JSON.stringify({ organization_id: organizationId, agent_id: context.agentId || null, reason: text(input.reason) || "Runtime requested platform repair review.", priority: text(input.priority) || "high", status: "open", notes: `Phase 13 execution ${context.executionId}` }) });
    return rows[0] || null;
  });

  register("leo.platform.organizations.read", async (input) => supabaseServerRequest(`organizations?select=id,name,slug,status,created_at,updated_at&order=updated_at.desc&limit=${Math.max(1, Math.min(100, numeric(input.limit, 50)))}`));
  register("leo.platform.tenant.pause", async (input) => {
    const organizationId = requireId(input, "organization_id", "tenant_id");
    const rows = await supabaseServerRequest<unknown[]>(`organizations?id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "paused", updated_at: new Date().toISOString() }) });
    if (!rows[0]) throw new Error("Organization was not found.");
    return rows[0];
  });
  register("leo.platform.tenant.resume", async (input) => {
    const organizationId = requireId(input, "organization_id", "tenant_id");
    const rows = await supabaseServerRequest<unknown[]>(`organizations?id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", body: JSON.stringify({ status: "active", updated_at: new Date().toISOString() }) });
    if (!rows[0]) throw new Error("Organization was not found.");
    return rows[0];
  });
  register("leo.platform.workflow.activate", async (input) => {
    const workflowId = requireId(input, "workflow_id", "id");
    const rows = await supabaseServerRequest<unknown[]>(`workflow_registry?id=eq.${encodeURIComponent(workflowId)}`, { method: "PATCH", body: JSON.stringify({ status: "active", updated_at: new Date().toISOString() }) });
    if (!rows[0]) throw new Error("Workflow was not found.");
    return rows[0];
  });
  register("leo.platform.workflow.deactivate", async (input) => {
    const workflowId = requireId(input, "workflow_id", "id");
    const rows = await supabaseServerRequest<unknown[]>(`workflow_registry?id=eq.${encodeURIComponent(workflowId)}`, { method: "PATCH", body: JSON.stringify({ status: "inactive", updated_at: new Date().toISOString() }) });
    if (!rows[0]) throw new Error("Workflow was not found.");
    return rows[0];
  });
  ...
  register("leo.limitless.leads.read", async (input, context) => {
    const organizationId = requireOrganization(context);
    return supabaseServerRequest(`leads?select=*&organization_id=eq.${encodeURIComponent(organizationId)}&order=updated_at.desc&limit=${Math.max(1, Math.min(100, numeric(input.limit, 25)))}`);
  });
