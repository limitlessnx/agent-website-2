...
async function getAccessToken() {
  const { clientEmail, privateKey } = driveConfig();
  if (!clientEmail || !privateKey) throw new Error("Google Drive is not configured. Add service account email and private key in Vercel.");
  const now = Math.floor(Date.now() / 1000);
  const header = base64Url(JSON.stringify({ alg: "RS256", typ: "JWT" }));
  const claim = base64Url(JSON.stringify({ iss: clientEmail, scope: driveScope, aud: tokenUrl, exp: now + 3600, iat: now }));
...
async function uploadDriveFile(file: File, folderId: string, accessToken: string) {
  ...
  const metadata = { name: file.name || `property-image-${Date.now()}`, parents: [folderId] };
...
export async function uploadSinglePropertyImageToDrive(propertyTitle: string, file: File, existingFolderId?: string) {
  ...
  let folderLink = folderId ? `https://drive.google.com/drive/folders/${folderId}` : "";

  if (!folderId) {
    const metadata = { name: `${propertyTitle || "Property"} - ${new Date().toISOString().slice(0, 10)}`, mimeType: "application/vnd.google-apps.folder" };
    let folder: DriveFile;
    try {
      folder = await createDriveFile({ ...metadata, ...(parentFolderId ? { parents: [parentFolderId] } : {}) }, accessToken);
    } catch (error) {
      if (!parentFolderId) throw error;
      folder = await createDriveFile(metadata, accessToken);
    }
    folderId = folder.id;
  ...
  await uploadDriveFile(file, folderId, accessToken);
...
export async function uploadPropertyImagesToDrive(propertyTitle: string, files: File[]) {
  ...
  for (const file of validFiles) {
    const result = await uploadSinglePropertyImageToDrive(propertyTitle, file, folderId || undefined);
