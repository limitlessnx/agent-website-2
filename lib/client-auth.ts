...
export type PendingClientSetupSession={userId:string;email:string;invitationToken?:string;issuedAt:number};
...
function readSignedToken<T>(token:string|undefined,ttlSeconds:number):T|null{if(!token)return null;const[payload,signature]=token.split(".");if(!payload||!signature||!safeEqual(signature,sign(payload)))return null;try{const value=JSON.parse(Buffer.from(payload,"base64url").toString("utf8")) as T&{issuedAt?:number};if(!value.issuedAt||Date.now()-value.issuedAt>ttlSeconds*1000)return null;return value}catch{return null}}
async function authRequest(path:string,body:Record<string,unknown>){const{url,anonKey,projectRef}=authConfig();let response:Response;try{response=await fetch(`${url}/auth/v1/${path}`,{method:"POST",headers:{apikey:anonKey,Authorization:`Bearer ${anonKey}`,"Content-Type":"application/json"},body:JSON.stringify(body),cache:"no-store"})}catch(error){console.error("Fluxknight Supabase Auth connection failed",{projectRef,error:error instanceof Error?error.message:String(error)});throw new Error("Account service is temporarily unavailable.")}const result=(await response.json().catch(()=>({}))) as SupabaseAuthResponse;if(!response.ok)throw new Error(result.error_description||result.msg||result.error||"Authentication failed.");return result}

export async function signUpClient(email:string,password:string,fullName:string,onboarding?:{companyName?:string;companySlug?:string;templateSlug?:string;agentFamilyName?:string}){
 return authRequest("signup",{email,password,data:{full_name:fullName,company_name:onboarding?.companyName||"",company_slug:onboarding?.companySlug||"",template_slug:onboarding?.templateSlug||"",agent_family_name:onboarding?.agentFamilyName||onboarding?.companyName||""}});
}
