...
export type ClientSession = {
  userId: string; email: string; organizationId: string; organizationSlug: string;
  membershipId: string; role: string; issuedAt: number;
};

export type PendingClientSetupSession = {
  userId: string; email: string; invitationToken?: string; issuedAt: number;
};
...
function projectRefFromUrl(value: string) {
  try { return new URL(value).hostname.match(/^([a-z0-9]+)\.supabase\.co$/i)?.[1] || ""; } catch { return ""; }
}
function projectRefFromKey(value: string) {
  if (!value || !value.includes(".")) return "";
  try { return String((JSON.parse(Buffer.from(value.split(".")[1], "base64url").toString("utf8")) as { ref?: string }).ref || ""); } catch { return ""; }
}
function authConfig() {
  ...
  if (urlProjectRef && keyProjectRef && urlProjectRef !== keyProjectRef) throw new Error(`Fluxknight Supabase configuration mismatch: the URL belongs to project ${urlProjectRef}, but the authentication key belongs to ${keyProjectRef}.`);
  return { url, anonKey, projectRef: urlProjectRef || keyProjectRef };
}
function sessionSecret() { return process.env.CLIENT_SESSION_SECRET || process.env.ADMIN_SESSION_SECRET || process.env.LIMITLESS_ADMIN_PASSWORD || "development-client-session-secret"; }
...
function normalizeRelation<T>(value: T | T[] | null): T | null { if (!value) return null; return Array.isArray(value) ? value[0] || null : value; }
function createSignedToken(value: Record<string, unknown>) { const payload=Buffer.from(JSON.stringify(value)).toString("base64url"); return `${payload}.${sign(payload)}`; }
function readSignedToken<T>(token: string | undefined, ttlSeconds: number): T | null {
  if (!token) return null; const [payload,signature]=token.split(".");
  if (!payload || !signature || !safeEqual(signature,sign(payload))) return null;
  try { const value=JSON.parse(Buffer.from(payload,"base64url").toString("utf8")) as T & { issuedAt?: number }; if (!value.issuedAt || Date.now()-value.issuedAt>ttlSeconds*1000) return null; return value; } catch { return null; }
}
async function authRequest(path: string, body: Record<string, unknown>) {
  const { url, anonKey, projectRef } = authConfig(); let response: Response;
  try { response = await fetch(`${url}/auth/v1/${path}`, { method:"POST", headers:{ apikey:anonKey, Authorization:`Bearer ${anonKey}`, "Content-Type":"application/json" }, body:JSON.stringify(body), cache:"no-store" }); }
  catch (error) { console.error("Fluxknight Supabase Auth connection failed",{ projectRef,error:error instanceof Error?error.message:String(error) }); throw new Error("Account service is temporarily unavailable."); }
  const result=(await response.json().catch(()=>({}))) as SupabaseAuthResponse; if (!response.ok) throw new Error(result.error_description || result.msg || result.error || "Authentication failed."); return result;
}

export async function signUpClient(email:string,password:string,fullName:string,onboarding?:{ companyName?:string; companySlug?:string; templateSlug?:string; agentFamilyName?:string; invitationToken?:string; }) {
  return authRequest("signup",{ email,password,data:{ full_name:fullName, company_name:onboarding?.companyName||"", company_slug:onboarding?.companySlug||"", template_slug:onboarding?.templateSlug||"", agent_family_name:onboarding?.agentFamilyName||onboarding?.companyName||"", invitation_token:onboarding?.invitationToken||"" } });
}
...
function membershipSnapshot(membership: MembershipRow) {
  ...
  const role=normalizeRelation(membership.membership_roles?.[0]?.roles || null);
...
export async function getMembershipForOrganization(userId:string,organizationId:string){
  const rows=await supabaseServerRequest<MembershipRow[]>(`organization_memberships?user_id=eq.${encodeURIComponent(userId)}&organization_id=eq.${encodeURIComponent(organizationId)}&status=eq.active&select=id,organization_id,status,organizations(slug),membership_roles(roles(slug))&limit=1`);
  return rows[0]?membershipSnapshot(rows[0]):null;
}
export async function getPrimaryMembership(userId:string){
  const rows=await supabaseServerRequest<MembershipRow[]>(`organization_memberships?user_id=eq.${encodeURIComponent(userId)}&status=eq.active&select=id,organization_id,status,organizations(slug),membership_roles(roles(slug))&order=created_at.asc&limit=1`);
  return rows[0]?membershipSnapshot(rows[0]):null;
}
