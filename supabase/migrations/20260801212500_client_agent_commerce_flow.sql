...
alter table public.organization_quotes add constraint organization_quotes_org_id_id_unique unique (organization_id, id);
alter table public.organization_agent_selections add constraint organization_agent_selections_org_id_id_unique unique (organization_id, id);
alter table public.custom_agent_requests add constraint custom_agent_requests_org_id_id_unique unique (organization_id, id);
...
create policy "members_manage_agent_selections" on public.organization_agent_selections for all to authenticated
using (public.is_organization_member(organization_id))
with check (public.is_organization_member(organization_id));

create policy "members_view_quotes" on public.organization_quotes for select to authenticated
using (public.is_organization_member(organization_id));

create policy "members_view_quote_items" on public.organization_quote_items for select to authenticated
using (public.is_organization_member(organization_id));

create policy "members_manage_custom_requests" on public.custom_agent_requests for all to authenticated
using (public.is_organization_member(organization_id))
with check (public.is_organization_member(organization_id) and submitted_by = auth.uid());

create policy "members_view_custom_proposals" on public.custom_agent_proposals for select to authenticated
using (public.is_organization_member(organization_id));
