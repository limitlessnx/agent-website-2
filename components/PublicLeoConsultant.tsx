...
export default function PublicLeoConsultant() {
  ...
  return (
    <aside className={`public-leo ${open ? "open" : ""}`} aria-label="Fluxknight Leo support assistant">
      ...
      <button
        type="button"
        className={`public-leo-launcher ${open ? "" : "attention"}`}
        onClick={toggleLeo}
        aria-label={open ? "Close Leo" : "Open Leo"}
        aria-expanded={open}
      >
        <span className="public-leo-pulse" aria-hidden="true" />
        <span className="public-leo-pulse secondary" aria-hidden="true" />
        <span className="public-leo-status-dot" aria-hidden="true" />
        <MessageCircle size={22} />
      </button>

      <style jsx>{`
        .public-leo{position:fixed;right:20px;bottom:20px;z-index:80;color:#f8fbff;font-family:inherit}
        .public-leo-launcher{position:relative;width:62px;height:62px;display:grid;place-items:center;padding:0;border:1px solid rgba(226,232,240,.3);border-radius:999px;color:#fff;background:linear-gradient(145deg,rgba(226,232,240,.2),rgba(124,58,237,.68) 42%,rgba(15,23,42,.84));box-shadow:0 18px 55px rgba(15,23,42,.38),0 0 18px rgba(167,139,250,.28),0 0 34px rgba(124,58,237,.18),inset 0 1px 0 rgba(255,255,255,.3);backdrop-filter:blur(18px) saturate(1.35);cursor:pointer;transform-origin:center;transition:transform .18s ease,box-shadow .18s ease,filter .18s ease}
        .public-leo-launcher svg{position:relative;z-index:2}.public-leo-pulse{position:absolute;inset:-12px;border:4px solid rgba(245,243,255,.98);border-radius:inherit;opacity:0;pointer-events:none;box-shadow:0 0 20px rgba(245,243,255,.78),0 0 38px rgba(196,181,253,.72),0 0 64px rgba(139,92,246,.56)}.public-leo-pulse.secondary{inset:-24px;border-color:rgba(251,146,60,.72);box-shadow:0 0 28px rgba(251,146,60,.42),0 0 54px rgba(249,115,22,.28),0 0 74px rgba(139,92,246,.18)}.public-leo-status-dot{position:absolute;right:3px;top:5px;z-index:3;width:9px;height:9px;border:2px solid rgba(15,23,42,.9);border-radius:50%;background:#a78bfa;box-shadow:0 0 0 0 rgba(167,139,250,.46),0 0 16px rgba(167,139,250,.95);pointer-events:none}.public-leo-launcher.attention{animation:leoLauncherBreathe 4.8s ease-in-out infinite}.public-leo-launcher.attention .public-leo-pulse{animation:leoLauncherPulse 4.8s ease-out infinite}.public-leo-launcher.attention .public-leo-pulse.secondary{animation-delay:.42s}.public-leo-launcher.attention .public-leo-status-dot{animation:leoStatusPulse 2.4s ease-out infinite}.public-leo-launcher.attention:hover,.public-leo-launcher.attention:focus-visible{animation-play-state:paused;transform:scale(1.03);filter:brightness(1.06);box-shadow:0 20px 62px rgba(76,29,149,.34),0 0 28px rgba(139,92,246,.18),inset 0 1px 0 rgba(255,255,255,.3)}.public-leo-launcher.attention:hover .public-leo-pulse,.public-leo-launcher.attention:focus-visible .public-leo-pulse,.public-leo-launcher.attention:hover .public-leo-status-dot,.public-leo-launcher.attention:focus-visible .public-leo-status-dot{animation-play-state:paused}
        ...
        @keyframes leoLauncherBreathe{0%,56%,100%{transform:scale(1);box-shadow:0 18px 55px rgba(15,23,42,.38),inset 0 1px 0 rgba(255,255,255,.28)}64%{transform:scale(1.04);box-shadow:0 22px 70px rgba(76,29,149,.4),0 0 36px rgba(139,92,246,.3),0 0 54px rgba(139,92,246,.16),inset 0 1px 0 rgba(255,255,255,.32)}72%{transform:scale(1.018)}80%{transform:scale(1.045);box-shadow:0 24px 74px rgba(76,29,149,.42),0 0 42px rgba(139,92,246,.34),0 0 62px rgba(249,115,22,.08),inset 0 1px 0 rgba(255,255,255,.32)}}@keyframes leoLauncherPulse{0%,50%{transform:scale(.94);opacity:0}57%{opacity:1}80%{transform:scale(1.98);opacity:0}100%{transform:scale(1.98);opacity:0}}@keyframes leoStatusPulse{0%,100%{box-shadow:0 0 0 0 rgba(167,139,250,.46),0 0 16px rgba(167,139,250,.95)}55%{box-shadow:0 0 0 9px rgba(167,139,250,0),0 0 22px rgba(167,139,250,1)}}@keyframes leoDot{0%,100%{transform:translateY(0);opacity:.45}50%{transform:translateY(-3px);opacity:1}}
        @media(max-width:520px){.public-leo{right:14px;bottom:14px}.public-leo-panel{width:min(360px,calc(100vw - 28px));max-height:calc(100vh - 90px)}}@media(prefers-reduced-motion:reduce){.public-leo-launcher.attention,.public-leo-launcher.attention .public-leo-pulse,.public-leo-launcher.attention .public-leo-status-dot{animation:none!important}.public-leo-launcher.attention{box-shadow:0 18px 55px rgba(15,23,42,.38),0 0 18px rgba(139,92,246,.1),inset 0 1px 0 rgba(255,255,255,.28)}}
      `}</style>
    </aside>
  );
}
