...
const streams = Array.from({ length: 25 }, (_, i) => i);
const particles = Array.from({ length: 42 }, (_, i) => ({ left: `${(i * 37) % 100}%`, top: `${(i * 61) % 100}%`, delay: (i % 9) * .22, duration: 5 + (i % 6) * .6 }));
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#03000A] text-neutral-50 selection:bg-purple-500/30">
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        ...
        <motion.div className="absolute -right-[22rem] top-[-10rem] h-[48rem] w-[48rem] rounded-full border-[10px] border-[#ff5500]/10 blur-md sm:-right-[15rem] lg:-right-[10rem]" animate={reduceMotion ? undefined : { scale: [1,1.02,1] }} transition={reduceMotion ? undefined : { duration: 4, repeat: Infinity, ease: "easeInOut" }} />
        ...
        <svg className="absolute -bottom-24 -right-[38rem] h-[42rem] w-[88rem] opacity-65 sm:-right-[24rem] lg:-right-[6rem]" viewBox="0 0 1200 600" fill="none">
          {streams.map((i) => <motion.path key={i} d={`M${270 + i * 5} 630 C ${350 + i * 3} ${510 - i * 2}, ${560 + i * 5} ${430 - i * 4}, ${745 + i * 7} ${315 - (i % 5) * 8} C ${900 + i * 4} ${225 + (i % 4) * 9}, 1060 ${165 + (i % 6) * 6}, 1240 ${125 + (i % 5) * 5}`} stroke={streamColors[i % streamColors.length]} strokeWidth={i % 6 === 0 ? 1.8 : .75} strokeLinecap="round" initial={reduceMotion ? false : { pathLength: 0, opacity: 0 }} animate={reduceMotion ? { opacity: .4 } : { pathLength: 1, opacity: [.12,.72,.2] }} transition={reduceMotion ? undefined : { duration: 4.8 + i * .13, repeat: Infinity, repeatType: "reverse", ease: "easeInOut", delay: i * .05 }} />)}
        </svg>
        {!reduceMotion && particles.map((p, i) => <motion.span key={i} className="absolute h-[2px] w-[2px] rounded-full bg-[#aa88ff]/60" style={{left:p.left,top:p.top}} animate={{opacity:[.08,.65,.08],y:[0,-5,0]}} transition={{duration:p.duration,repeat:Infinity,ease:"easeInOut",delay:p.delay}} />)}
      ...
      <div className="relative z-10 mx-auto grid min-h-[85vh] max-w-7xl grid-cols-1 items-center gap-12 px-6 pb-32 pt-24 lg:grid-cols-12 lg:gap-8 lg:pb-40 lg:pt-32">
        <div className="z-20 flex flex-col gap-6 lg:col-span-5 lg:gap-8">
          <motion.div initial={reduceMotion ? false : {opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:.6}} className="flex items-start">
          ...
          <h1 className="text-5xl font-medium leading-[1.1] tracking-tight md:text-6xl lg:text-7xl">
            {["Grow your organization", "without growing", "the workload."].map((line,index)=><div key={line} className="overflow-hidden pb-1"><motion.span className={index===2?"block bg-gradient-to-r from-orange-300 via-pink-400 to-purple-400 bg-clip-text text-transparent":"block"} initial={reduceMotion?false:{y:"110%"}} animate={{y:0}} transition={{duration:1.2,delay:reduceMotion?0:.2+index*.15,ease:[.22,1,.36,1]}}>{line}</motion.span></div>)}
          </h1>

          <motion.p initial={reduceMotion?false:{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:1,delay:reduceMotion?0:.8}} className="max-w-md text-lg font-normal leading-relaxed text-neutral-400">Fluxknight builds AI systems that handle customer conversations and the work that follows, from enquiry and support to follow-up, scheduling, CRM updates, and human handoff.</motion.p>

          <motion.div initial={reduceMotion?false:{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:1,delay:reduceMotion?0:.9}} className="flex flex-col items-center gap-4 pt-2 sm:flex-row">
        ...
        <motion.div initial={reduceMotion?false:{opacity:0,y:40,rotateY:-20,rotateX:10}} animate={{opacity:1,y:0,rotateY:-12,rotateX:4}} transition={{duration:1.8,delay:reduceMotion?0:.6,ease:[.22,1,.36,1]}} className="relative z-20 w-full lg:col-span-7" style={{perspective:"1200px",transformStyle:"preserve-3d"}}>
