...
export default function LeoFloatingButton() {
  ...
  const statusLabel = operationState === "investigating" ? "Investigating" : operationState === "error" ? "Needs attention" : "Ready";
  const statusDot = operationState === "error" ? "bg-rose-400" : operationState === "investigating" ? "bg-amber-300" : "bg-emerald-400";
    ...
    <aside id="leo-assistant-panel" aria-label="Agent Leo" aria-modal="false" className={`fixed bottom-20 right-6 z-[119] flex h-[min(680px,calc(100vh-110px))] w-[min(440px,calc(100vw-28px))] flex-col overflow-hidden rounded-2xl border border-violet-300/20 bg-slate-950 text-white shadow-2xl transition-all duration-200 max-md:bottom-[146px] max-md:right-3 max-md:h-[min(460px,calc(100dvh-170px))] max-md:w-[calc(100vw-24px)] ${open ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-3 opacity-0"}`}>
      ...
      <div className="relative min-h-0 flex-1">
        ...
        {!voice ? <div className="absolute inset-0 flex flex-col">
          <div className="min-h-0 flex-1 overflow-y-auto p-4 text-xs">{!messages.length ? <div className="flex h-full flex-col items-center justify-center text-center text-slate-400"><Bot size={28} className="mb-3 text-violet-300" /><strong className="text-base text-white">What needs attention?</strong><p className="mt-2 max-w-xs leading-5">Leo understands this page and can investigate Fluxknight in context.</p></div> : messages.map((item, index) => <div key={`${item.role}-${index}`} className={`mb-3 ${item.role === "user" ? "text-right" : "text-left"}`}><span className="mb-1 block text-[9px] font-bold tracking-wider text-slate-500">{item.role === "user" ? "YOU" : "LEO"}{item.source === "voice" ? " · VOICE" : ""}</span><div className="inline-block max-w-[90%] whitespace-pre-wrap rounded-xl border border-white/5 bg-white/[0.03] px-3 py-2.5 leading-5">{item.content}</div></div>)}{busy ? <div className="flex items-center gap-2 rounded-lg border border-amber-300/10 bg-amber-300/[0.04] p-2 text-[10px] text-amber-100"><Loader2 size={13} className="animate-spin" />Leo is investigating the current state...</div> : null}{error ? <div className="mt-3 rounded-lg border border-rose-300/20 bg-rose-500/10 p-2 text-[10px] text-rose-200" role="alert">{error}</div> : null}</div>
          <div className="mx-3 mb-2 rounded-lg border border-white/5 bg-white/[0.025] px-3 py-2 text-[9px] text-slate-400"><strong className="text-slate-200">Operational mode:</strong> observe → investigate → recommend → request approval → act → verify.</div>
