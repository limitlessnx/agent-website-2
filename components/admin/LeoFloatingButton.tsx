...
export default function LeoFloatingButton() {
  ...
  const statusLabel = operationState === "investigating" ? "Investigating" : operationState === "executing" ? "Executing" : operationState === "approval_required" ? "Approval required" : operationState === "completed" ? "Executed" : operationState === "error" ? "Needs attention" : "Ready";
  const statusDot = operationState === "error" ? "bg-rose-400" : operationState === "investigating" || operationState === "executing" ? "bg-amber-300" : operationState === "approval_required" ? "bg-orange-300" : "bg-emerald-400";
    ...
    <aside id="leo-assistant-panel" aria-label="Agent Leo" aria-modal="false" className={`fixed bottom-20 right-6 z-[119] flex h-[min(680px,calc(100vh-110px))] w-[min(440px,calc(100vw-28px))] flex-col overflow-hidden rounded-2xl border border-violet-300/20 bg-slate-950 text-white shadow-2xl transition-all duration-200 max-md:bottom-[146px] max-md:right-3 max-md:h-[min(460px,calc(100dvh-170px))] max-md:w-[calc(100vw-24px)] ${open ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-3 opacity-0"}`}>
      ...
      <div className="relative min-h-0 flex-1">
        ...
        {!voice ? <div className="absolute inset-0 flex flex-col">
          ...
          <div className="mx-3 mb-2 rounded-lg border border-white/5 bg-white/[0.025] px-3 py-2 text-[9px] text-slate-400"><strong className="text-slate-200">Operational mode:</strong> observe → investigate → recommend → request approval → act → verify with evidence.</div>
