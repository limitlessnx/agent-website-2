...
export default function LeoFloatingButton() {
  ...
  return <>
    <button type="button" aria-label="Open Agent Leo" onClick={() => setOpen(true)} className="fixed bottom-6 right-6 z-[120] flex h-12 min-w-12 items-center justify-center gap-2 rounded-full border border-violet-300/30 bg-slate-950 px-4 text-white shadow-2xl transition hover:-translate-y-0.5 hover:border-violet-200/60 focus:outline-none focus:ring-2 focus:ring-violet-300/50"><Bot size={17} /><span className="text-xs font-semibold">Leo</span></button>
    <aside aria-label="Agent Leo" className={`fixed bottom-20 right-6 z-[119] flex h-[min(680px,calc(100vh-110px))] w-[min(440px,calc(100vw-28px))] flex-col overflow-hidden rounded-2xl border border-violet-300/20 bg-slate-950 text-white shadow-2xl transition-all duration-200 ${open ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-3 opacity-0"}`}>
