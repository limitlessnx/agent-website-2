...
export default function LeoFloatingButton() {
  ...
  useEffect(() => {
    if (open && !voice) messageRef.current?.focus();
  }, [open, voice]);
  ...
  return <>
    <button type="button" aria-label="Open Agent Leo" aria-expanded={open} aria-controls="leo-assistant-panel" onClick={() => setOpen(true)} className="fixed bottom-6 right-6 z-[120] flex h-12 min-w-12 items-center justify-center gap-2 rounded-full border border-violet-300/30 bg-slate-950 px-4 text-white shadow-2xl transition hover:-translate-y-0.5 hover:border-violet-200/60 focus:outline-none focus:ring-2 focus:ring-violet-300/50 max-md:bottom-[82px] max-md:right-3"><Bot size={17} /><span className="text-xs font-semibold">Leo</span></button>
    <aside id="leo-assistant-panel" aria-label="Agent Leo" aria-modal="false" className={`fixed bottom-20 right-6 z-[119] flex h-[min(680px,calc(100vh-110px))] w-[min(440px,calc(100vw-28px))] flex-col overflow-hidden rounded-2xl border border-violet-300/20 bg-slate-950 text-white shadow-2xl transition-all duration-200 max-md:bottom-[146px] max-md:right-3 max-md:h-[min(460px,calc(100dvh-170px))] max-md:w-[calc(100vw-24px)] ${open ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-3 opacity-0"}`}>
      <header className="flex items-center justify-between border-b border-white/10 px-4 py-3"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl border border-violet-300/20 bg-violet-500/10"><Bot size={18} /></span><div><strong className="block text-sm">Agent Leo</strong><span className="text-[10px] text-slate-400">Fluxknight operator · {context.section}</span></div></div><button type="button" onClick={closeLeo} aria-label="Close Leo" className="rounded-lg p-2 text-slate-400 hover:bg-white/5 hover:text-white focus:outline-none focus:ring-2 focus:ring-violet-300/50"><X size={17} /></button></header>
      ...
      <div className="relative min-h-0 flex-1">
        ...
        {!voice ? <div className="absolute inset-0 flex flex-col">
          ...
          <form className="border-t border-white/10 bg-slate-950 p-3" onSubmit={(event) => { event.preventDefault(); void sendMessage(event.currentTarget); }}><div className="flex items-end gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-2"><textarea ref={messageRef} name="message" rows={1} required placeholder="Ask Leo about this page..." aria-label="Message Leo" className="min-h-8 flex-1 resize-none bg-transparent text-xs text-white outline-none placeholder:text-slate-600" /><button type="button" onClick={() => setVoice(true)} aria-label="Start Leo voice call" className="grid h-8 w-8 place-items-center rounded-lg border border-violet-300/20 bg-violet-500/10 text-violet-200 focus:outline-none focus:ring-2 focus:ring-violet-300/50"><Mic size={15} /></button><button type="submit" disabled={busy} aria-label="Send" className="grid h-8 w-8 place-items-center rounded-lg bg-violet-600 text-white disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-violet-300/50"><Send size={15} /></button></div><div className="mt-2 flex items-center gap-1 text-[8px] text-slate-600"><ShieldCheck size={10} />Sensitive actions remain approval-gated.</div></form>
        </div> : null}
      </div>
    </aside>
  </>;
