...
import { recommendationForLeoSignal } from "@/lib/leo-proactive-policy";
...
export default function LeoProactiveSignals({ initialSignals }: { initialSignals: LeoPersistedSignal[] }) {
  ...
  async function acknowledge(signalId: string) {
    setBusy(signalId);
    try {
      const response = await fetch("/api/leo/monitor", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "acknowledge", signalId }) });
  ...
  return <div className="admin-list">
    {signals.map((item) => {
      const analysis = recommendationForLeoSignal(item);
      return <div key={item.id} className="admin-list-row">
        <Link href={item.href} style={{ minWidth: 0, flex: 1, textDecoration: "none", color: "inherit" }}>
          <div><strong>{item.title}</strong><span>{item.summary}</span><span><b>Leo recommends:</b> {analysis.safeNextStep}</span><span style={{ opacity: 0.72 }}>Boundary: {analysis.consequenceBoundary}</span></div>
        </Link>
