      ...
      <div className={styles.shell}>
        <div className={styles.headingRow}>
          ...
          <div className={styles.controls} aria-label="Industry carousel controls">
            <button type="button" onClick={() => goTo(active - 1)} aria-label="Previous industry"><ArrowLeft size={18} /></button>
            <span>{String(active + 1).padStart(2, "0")} / {String(industries.length).padStart(2, "0")}</span>
            <button type="button" onClick={() => goTo(active + 1)} aria-label="Next industry"><ArrowRight size={18} /></button>
          </div>
        </div>
        ...
        <div className={styles.mobileNav} aria-label="Industry carousel navigation">
          <button type="button" onClick={() => goTo(active - 1)} aria-label="Previous industry"><ArrowLeft size={19} /></button>
          <div className={styles.dots}>{industries.map((item, index) => <button key={item.id} type="button" className={index === active ? styles.dotActive : styles.dot} onClick={() => goTo(index)} aria-label={`Show ${item.title}`} aria-pressed={index === active} />)}</div>
          <button type="button" onClick={() => goTo(active + 1)} aria-label="Next industry"><ArrowRight size={19} /></button>
        </div>
