  ...
  return (
    ...
    >
      <div className={styles.topControls}>
        ...
        <div>
          <button type="button" onClick={() => move(-1)} disabled={active === 0} aria-label="Previous pricing plan">
            <ArrowLeft size={18} />
          </button>
          <button type="button" onClick={() => move(1)} disabled={active === plans.length - 1} aria-label="Next pricing plan">
            <ArrowRight size={18} />
          </button>
        </div>
      </div>
      ...
      <div
        ...
        tabIndex={0}
        role="region"
        aria-label="Scrollable pricing plans"
      >
        {plans.map((plan, index) => {
          ...
          return (
            <article
              ...
              key={plan.name}
              aria-label={`${plan.name}${plan.featured ? ", most complete starter" : ""}`}
            >
              ...
              <div className={styles.priceBlock}>
                <div>
                  <span>First month · installation + deployment</span>
                  <strong>{firstPrice}</strong>
                </div>
                <div>
                  <span>Consecutively</span>
                  <strong>{plan.ongoing}</strong>
                </div>
              </div>
              ...
              <Link className={styles.cta} href={href} aria-label={`${plan.cta ?? "Get started"} with ${plan.name}`}>
                {plan.cta ?? "Get started"} <ArrowRight size={16} />
              </Link>
            </article>
          );
        })}
